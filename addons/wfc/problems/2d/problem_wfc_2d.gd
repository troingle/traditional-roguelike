...
func split(concurrency_limit: int) -> Array[SubProblem]:
	...
	var max_range = Vector2i(100, 100)
	if influence_range.x > max_range.x:
		influence_range.x = max_range.x
	if influence_range.y > max_range.y:
		influence_range.y = max_range.y
